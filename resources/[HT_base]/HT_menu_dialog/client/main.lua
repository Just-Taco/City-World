Citizen.CreateThread(function()
	...
	local openMenu = function(namespace, name, data)
		...
		--local timeoutId = HT.SetTimeout(200, function()
	...
	HT.UI.Menu.RegisterType(MenuType, openMenu, closeMenu)

	RegisterNUICallback('menu_submit', function(data, cb)
		local menu = HT.UI.Menu.GetOpened(MenuType, data._namespace, data._name)
		local post = true
		...
		if menu.submit ~= nil then
			...
			if tonumber(data.value) ~= nil then
				...
				-- Round float values
				data.value = HT.Math.Round(tonumber(data.value))
			...
			data.value = HT.Math.Trim(data.value)
